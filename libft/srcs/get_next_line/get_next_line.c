...
/*   Updated: 2023/07/08 12:41:59 by lwoiton          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int file_des)
{
	...
	ln_break = ft_strchr_gnl(buff[file_des], '\n');
	line = ft_substr_gnl(buff[file_des], ln_break);
...
char	*read_and_concat_buff(int file_des, char *buff)
{
	...
	if (!read_buff)
		return (NULL);
	while (bytes_read && !ft_strchr_gnl(buff, '\n'))
	{
		...
		buff = ft_strjoin_gnl(buff, read_buff);
	}
	free(read_buff);
	return (buff);
}
...
char	*realloc_buff(char *buff, char *ln_break)
{
	...
	copy_size = ft_strlen_gnl(ln_break) - 1;
	new_buff = (char *) malloc(copy_size + 1);
	if (!new_buff)
		return (NULL);
	ft_strlcpy_gnl(new_buff, ln_break + 1, copy_size + 1);
	free(buff);
	return (new_buff);
}
